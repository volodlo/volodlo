...
def data_fill():
    ...
    try:
        Courses.insert_many(courses_data,
                            fields=[Courses.id, Courses.name, Courses.time_start, Courses.time_end]).execute()
    except IntegrityError:
        print('table filling skipped...')
    try:
        Students.insert_many(students_data, fields=[Students.id, Students.name, Students.surname, Students.age,
                                                    Students.city]).execute()
    except IntegrityError:
        print('table filling skipped...')
    try:
        StudentCourses.insert_many(student_courses_data,
                                   fields=[StudentCourses.student_id, StudentCourses.courses_id]).execute()
    except IntegrityError:
        print('table filling skipped...')
